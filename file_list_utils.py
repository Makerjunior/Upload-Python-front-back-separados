...
def get_file_list():
    ...
    for f in files:
        ...
        elif ext.lower() == '.iso':  # Adicione a condição para arquivos .iso
            file_groups.setdefault('ISO', []).append(f'/arquivos/{f}')
        else:
            file_groups.setdefault('Outros', []).append(f'/arquivos/{f}')
    
    file_list = ''
    for ext, urls in file_groups.items():
        if ext in ['PDF', 'TXT', 'Word', 'Excel', 'PowerPoint', 'Imagem', 'MP3', 'Áudio', 'Executável', 'ZIP']:
            ...
            for url in urls:
                filename = os.path.basename(url)
                file_list += f'<video width="320" height="240" controls style="margin: 15px;"><source src="{url}" type="video/mp4"></video>'
            file_list += '</ul>'
        elif ext == 'ISO':
            file_list += f'<h3>{ext}</h3><ul>'
            for url in urls:
